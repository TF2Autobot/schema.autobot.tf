...
export function fixItem(item: Item, schema: Schema) {
    ...
    if (schemaItem.name.includes(schemaItem.item_class.toUpperCase())) {
        for (let i = 0; i < itemsCount; i++) {
            if (
                items[i].item_class === schemaItem.item_class &&
                items[i].name.startsWith('Upgradeable ')
            ) {
    ...
    if (isPromo && item.quality != 1) {
        for (let i = 0; i < itemsCount; i++) {
            if (
                !isPromoItem(items[i]) &&
                items[i].item_name == schemaItem.item_name
            ) {
    ...
    if (schemaItem.item_class === 'supply_crate') {
        ...
        if (series === null) {
            ...
            if (
                ...
                itemsGameItem.static_attrs['set supply crate series'] !==
                    undefined
            ) {
                if (
                    isObject(
...
export function isPromoItem(schemaItem) {
    return (
        schemaItem.name.startsWith('Promo ') && schemaItem.craft_class === ''
    );
}
